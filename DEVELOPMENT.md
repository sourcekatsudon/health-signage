...
実画面のチェック：1920×620・1366×620のChrome viewportでスクロールなし、全ボタン44px以上（通常の高さでは48px以上）、7入力、14日グラフ、過去日と今日への切替、前日コピー。低エネルギー・低気分・低現実対処力で通常サジェスト、死にたい気持ち2で危険候補除外、3で通常候補停止。Notionの実DB設定後は未記録日のCREATEと同日UPDATE（行数・page IDが同一）、ネットワーク停止中の保存と復旧後の同期を確認する。
