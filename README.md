...
本人固有の悪化の兆候を継続して観測する、ローカルMac用の計器盤。1920×620、Chrome、タッチ操作、Asia/Tokyo基準。左70%が14日グラフ、右30%が7項目の入力です。
...
`npm start` はビルド後にWaitressを起動します。待ち受けは `.env.local` の `HEALTH_HOST` / `HEALTH_PORT`（既定値：127.0.0.1 / 5000）で指定します。Chromeで設定したアドレスを開き、表示領域1920×620・ズーム100%・全画面表示を使用してください。画面外寸にはChromeのタブやツールバーを含めないでください。 高さ700px以下では余白と各エリアを自動でコンパクトにします。高さ660px以下ではボタンを44pxにして、620pxの画面でも下端に余裕を残して7項目とフッターを収めます。さらに低い画面では入力欄をスクロールできます。`localhost` と `127.0.0.1` はlocalStorageが別なので、普段使うURLを固定します。
...
上部の「横」「縦」ボタンで表示を切り替えます。横モードは従来のグラフ・入力欄の左右配置です。縦モードは1920×640の出力を反時計回りに90度回転し、640×1920の縦置き用にグラフと入力欄を上下に配置します。横画面上で文字は下から上に進む向きになります。OS側ですでに640×1920の縦向き出力になっている場合は、追加の回転をせず縦配置だけ適用します。表示モードはブラウザに保存され、再読み込み後も維持されます。

Android 7.1の標準ブラウザ・サイネージ内WebView向けに、Chrome/WebView 53を互換性の基準にした通常のスクリプトを配信します。古い端末ではGrid・flex gapを使わない配置に自動切り替えし、画面サイズをピクセルで計算します。縦表示は640×1920を保ったまま表示領域へ縮小するため、端末の表示倍率やブラウザのバーがあっても全体を表示できます。必要な互換処理はアプリに同梱しており、外部CDNへの接続は不要です。旧APIを無効化したブラウザ検証は実施済みですが、Android 7.1実機の表示・タッチ操作は未検証です。
